      ...
      <div className="space-y-4 pb-8">
        ...
        {stats && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
            ...
            <Card className="p-3 text-center bg-card border-[var(--border)] shadow-xs rounded-xl"><div className="text-lg font-bold font-mono text-emerald-700">{stats.resolved}</div><div className="text-[10px] text-muted-foreground uppercase font-mono">Resolved</div></Card>
          </div>
        )}
      ...
      {isLoading ? <Skeleton className="h-64 rounded-xl bg-muted" /> : incidents.length === 0 ? <Card className="bg-card border-[var(--border)] shadow-xs rounded-xl"><CardContent className="flex flex-col items-center justify-center py-16 text-center"><ShieldCheck className="h-12 w-12 text-[var(--primary)] mb-3" /><p className="text-sm text-muted-foreground">No safety records. Stay safe!</p></CardContent></Card> : (
        <div className="space-y-2">{incidents.map(inc => { const Icon = TYPE_ICONS[inc.type] ?? AlertTriangle; return (
          <Card key={inc.id} className="bg-card border-[var(--border)] shadow-xs hover:border-[var(--primary)] transition-all rounded-xl"><CardContent className="p-3 flex items-start gap-3">
            ...
            <div className="shrink-0"><span className={cn("rounded px-1.5 py-0.5 text-[9px] font-bold", inc.status === "closed" || inc.status === "resolved" ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-amber-50 text-amber-700 border border-amber-200")}>{inc.status}</span>{inc.status !== "closed" && inc.status !== "resolved" && <button onClick={() => statusMut.mutate({ id: inc.id, status: "resolved" })} className="block text-[9px] text-[var(--primary)] font-bold hover:underline mt-1">Resolve</button>}</div>
          </CardContent></Card>
        ); })}</div>
      )}
    </div>
