    ...
    () => [
      {
        ...
        render: (_val, row) => {
          ...
          return (
            <div>
              <div className="font-bold text-white leading-tight">{bsMiti}</div>
        ...
        render: (val, row) =>
          val ? (
            <div className="flex flex-col">
              <Badge variant="outline" className="text-[10px] px-1 py-0 h-4 font-bold text-emerald-400 border-white/10 w-fit">
                {val}
              </Badge>
            ...
            <span className="text-[10px] text-gray-400 font-mono">Chq: {row.chequeNo}</span>
          ) : (
      ...
      },
      {
        ...
        render: (val, row) => (
          <div>
            <div className="font-bold text-white truncate text-xs">{val || "General"}</div>
            {row.subCategory && (
              <div className="text-[10px] text-gray-400 truncate">↳ {row.subCategory}</div>
            )}
          </div>
        ),
      },
      {
        ...
        render: (val, row) => (
          <div>
            <div className="font-semibold text-white truncate max-w-[220px] text-xs">{val}</div>
      ...
      },
      {
        ...
        render: (val, row) => (
          <div>
            <div className="capitalize text-gray-300 font-medium text-xs">
              {val?.replace(/_/g, " ") || "—"}
            </div>
            {row.bankAccount && (
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-2.5 rounded-xl border border-white/10 bg-[#0c1015] text-xs font-mono">
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-2">
            <span className="text-gray-400">TDS Remitted:</span>
            <span className="font-bold text-amber-400">NPR {formatNpr(stats?.totalTds || 0)}</span>
          </div>
          <div className="h-3 w-[1px] bg-white/10" />
          <div className="flex items-center gap-2">
            <span className="text-gray-400">Net Cash Outflow:</span>
            <span className="font-bold text-emerald-400">NPR {formatNpr((stats?.totalPaid || 0) - (stats?.totalTds || 0))}</span>
          </div>
        </div>
            ...
            {canWrite && (
              <>
                <Button
                  ...
                  className="h-8 px-2.5 text-xs font-mono text-amber-400 border-amber-500/30 bg-[#121820] hover:bg-[#1a2330] rounded-lg gap-1"
                >
                ...
                <Button
                  ...
                  className="h-8 px-2.5 text-xs font-mono text-purple-400 border-purple-500/30 bg-[#121820] hover:bg-[#1a2330] rounded-lg gap-1"
                >
      ...
      <Dialog open={Boolean(viewScanUrl)} onOpenChange={() => setViewScanUrl(null)}>
        <DialogContent className="max-w-3xl max-h-[85vh] p-4 flex flex-col font-sans bg-[#0c1015] border-white/10 text-white">
          <DialogHeader>
            <DialogTitle className="text-sm font-bold">Attached Payment Voucher / Receipt</DialogTitle>
          </DialogHeader>
          <div className="flex-1 overflow-auto flex items-center justify-center p-2 bg-[#121820] rounded-xl border border-white/10 min-h-[300px]">
          ...
          <DialogFooter>
            <Button size="sm" onClick={() => setViewScanUrl(null)} className="h-8 text-xs bg-white/10 hover:bg-white/20 text-white rounded-xl">
              Close
            </Button>
          </DialogFooter>
        </DialogContent>
