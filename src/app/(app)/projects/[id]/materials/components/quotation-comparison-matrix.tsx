...
import { Printer, CheckCircle2, AlertTriangle, Building2, TrendingDown, Star } from "lucide-react";
...
export function QuotationComparisonMatrix({ requisitionNumber, requisitionDate, items }: QuotationComparisonMatrixProps) {
  const handlePrint = () => {
    window.print();
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b pb-3">
        ...
        <Button size="sm" variant="outline" onClick={handlePrint} className="gap-1.5 h-8 text-xs shrink-0 print:hidden">
        ...
        {items.map((item, itemIdx) => {
          ...
          return (
            <div key={item.id} className="rounded-xl border border-border/80 bg-card overflow-hidden shadow-sm">
              <div className="p-3.5 bg-muted/40 border-b flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-3 text-xs">
                  <span className="font-medium text-muted-foreground">
                    Required Quantity: <strong className="text-foreground font-mono">{item.quantity.toLocaleString()} {item.unit}</strong>
                  </span>
                </div>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead className="bg-muted/20 border-b">
                    <tr>
                      <th className="py-2.5 px-3 text-left font-medium text-muted-foreground">Vendor / Supplier</th>
                      <th className="py-2.5 px-3 text-right font-medium text-muted-foreground">Ex-Factory Rate</th>
                      <th className="py-2.5 px-3 text-right font-medium text-muted-foreground">Freight / Transport</th>
                      <th className="py-2.5 px-3 text-right font-medium text-muted-foreground">Landed Rate (NPR)</th>
                      <th className="py-2.5 px-3 text-right font-medium text-muted-foreground">Total Cost (NPR)</th>
                      <th className="py-2.5 px-3 text-center font-medium text-muted-foreground w-28">Evaluation</th>
                    </tr>
                  </thead>
                        ...
                        >
                          <td className="py-2.5 px-3">
                            <div className="flex items-center gap-1.5">
                              ...
                              {q.isSelected && (
                                <Badge variant="secondary" className="bg-emerald-100 text-emerald-800 dark:bg-emerald-900/60 dark:text-emerald-300 text-[10px] py-0 px-1.5 h-4">
                                  Selected
                                </Badge>
                              )}
                            </div>
                            {q.notes && <p className="text-[11px] text-muted-foreground pl-5">{q.notes}</p>}
                          </td>
                          <td className="py-2.5 px-3 text-right font-mono">
                            NPR {q.exFactoryRate.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                          </td>
                          <td className="py-2.5 px-3 text-right font-mono">
                            NPR {q.transportRate.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                          </td>
                          <td className="py-2.5 px-3 text-right font-mono font-bold text-foreground">
                            NPR {q.totalRate.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                          </td>
                          <td className="py-2.5 px-3 text-right font-mono">
                            NPR {totalCost.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                          </td>
                          <td className="py-2.5 px-3 text-center">
                            {q.isLowest ? (
                              <Badge className="bg-emerald-600 hover:bg-emerald-600 text-white font-mono text-[10px] gap-1 py-0 px-2">
              ...
              {isHigherThanLowest && (
                <div className="p-3 bg-amber-50/70 dark:bg-amber-950/30 border-t border-amber-200 dark:border-amber-900/60 flex items-start gap-2.5 text-xs text-amber-900 dark:text-amber-200">
