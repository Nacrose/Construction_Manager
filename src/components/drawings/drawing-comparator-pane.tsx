...
export function DrawingComparatorPane({ projectId }: { projectId: string }) {
  ...
  return (
    <div className="flex flex-col h-full bg-card border-l border-[var(--border)] font-mono text-xs overflow-hidden">
      {/* Header Tabs */}
      <div className="flex items-center border-b border-[var(--border)] bg-[#f8fbfe] p-2 gap-2">
        <button
          onClick={() => setTab("boq")}
        ...
        <button
          onClick={() => setTab("rfis")}
          className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs transition-colors ${
            tab === "rfis" ? "bg-cyan-500/20 text-info border border-info/40 font-bold" : "text-muted-foreground hover:text-foreground"
          }`}
        >
          <FileQuestion className="h-3.5 w-3.5 text-[var(--primary)]" /> Linked RFIs ({rfiData?.rfis.length ?? 0})
        </button>
      </div>

      {/* Search Input */}
      <div className="p-2 border-b border-[var(--border)] bg-card">
        <div className="relative">
          ...
          <Input
            ...
            className="h-8 pl-8 text-xs font-mono bg-[#f8fbfe] border-[var(--border)] text-foreground rounded-xl"
          />
        </div>
      </div>
            ...
            <div className="space-y-1.5">
              {filteredBoq.map((item) => (
                <div key={item.id} className="p-2.5 rounded-xl border border-[var(--border)] bg-[#f8fbfe]/80 hover:border-emerald-500/40 transition-all group">
                  ...
                  <div className="flex items-center justify-between mt-2 pt-1.5 border-t border-[var(--input)]">
            ...
            <div className="space-y-1.5">
              {filteredRfis.map((rfi) => (
                <Link
                  ...
                  className="block p-2.5 rounded-xl border border-[var(--border)] bg-[#f8fbfe]/80 hover:border-info/40 transition-all"
                >
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-bold text-[var(--primary)]">{rfi.number}</span>
                    <span className="text-[9px] uppercase px-1.5 py-0.5 rounded border border-[var(--border)] font-mono text-foreground/80">
                      {rfi.status}
                    </span>
                  </div>
                  <p className="text-[11px] text-foreground/80 truncate mt-1">{rfi.subject}</p>
                </Link>
              ))}
            </div>
          )
