...
export function InterSiteTransferDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[700px] bg-card border-[var(--border)] text-foreground shadow-2xl">
        <DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-2">
          {/* Mode Banner */}
          <div className="flex items-center justify-between p-3 rounded-xl border border-[var(--border)] bg-[#f8fbfe]">
            ...
            <div className="flex items-center gap-2">
              <Label htmlFor="instant-toggle" className="text-xs font-medium text-foreground/80">
                Instant Move
              </Label>
              <Switch
                id="instant-toggle"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <Label className="text-xs font-semibold text-foreground/90">Origin Site (From) *</Label>
            ...
            <div className="space-y-1">
              <Label className="text-xs font-semibold text-foreground/90">Destination Site (To) *</Label>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="col-span-1 space-y-1">
              <Label className="text-xs font-semibold text-foreground/90">Select Material *</Label>
            ...
            <div className="space-y-1">
              ...
              {selectedMaterial && (
                <div className="text-[10px] text-muted-foreground">
                  Current Stock: <span className="font-mono text-[var(--primary)] font-bold">{selectedMaterial.currentStock} {selectedMaterial.unit}</span>
                </div>
              )}
            </div>

            <div className="space-y-1">
              <Label className="text-xs font-semibold text-foreground/90">Internal Rate (NPR / unit)</Label>
              <Input
                type="number"
                step="any"
                value={transferRate}
              ...
              />
              {estTotal > 0 && (
                <div className="text-[10px] text-[var(--primary)] font-mono">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="space-y-1">
              <Label className="text-xs text-foreground/80">Vehicle / Truck No</Label>
              <Input
                value={vehicleNo}
            ...
            <div className="space-y-1">
              <Label className="text-xs text-foreground/80">Driver Name / Phone</Label>
              <Input
                value={driverName}
            ...
            <div className="space-y-1">
              <Label className="text-xs text-foreground/80">Dispatch Chalan No</Label>
              <Input
                value={chalanNo}
          ...
          {/* Remarks */}
          <div className="space-y-1">
            <Label className="text-xs text-foreground/80">Notes & Purpose</Label>
            <Textarea
              value={remarks}
              onChange={(e) => setRemarks(e.target.value)}
              rows={2}
              placeholder="e.g. Surplus cement transferred for retaining wall urgent casting."
              className="text-xs bg-card border-[var(--border)] text-foreground resize-none"
            />
          </div>
