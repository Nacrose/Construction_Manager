...
export function InterSiteTransferDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[700px] bg-white border-[#c7d8e8] text-slate-900 shadow-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-base font-bold text-slate-900">
            <ArrowRightLeft className="h-5 w-5 text-[#0284c7]" />
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-2">
          {/* Mode Banner */}
          <div className="flex items-center justify-between p-3 rounded-xl border border-[#c7d8e8] bg-[#f8fbfe]">
            ...
            <div className="flex items-center gap-2">
              <Label htmlFor="instant-toggle" className="text-xs font-medium text-slate-700">
                Instant Move
              </Label>
              <Switch
                id="instant-toggle"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <Label className="text-xs font-semibold text-slate-800">Origin Site (From) *</Label>
            ...
            <div className="space-y-1">
              <Label className="text-xs font-semibold text-slate-800">Destination Site (To) *</Label>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="col-span-1 space-y-1">
              <Label className="text-xs font-semibold text-slate-800">Select Material *</Label>
            ...
            <div className="space-y-1">
              ...
              {selectedMaterial && (
                <div className="text-[10px] text-slate-500">
                  Current Stock: <span className="font-mono text-[#0284c7] font-bold">{selectedMaterial.currentStock} {selectedMaterial.unit}</span>
                </div>
              )}
            </div>

            <div className="space-y-1">
              <Label className="text-xs font-semibold text-slate-800">Internal Rate (NPR / unit)</Label>
              <Input
                type="number"
                step="any"
                value={transferRate}
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="space-y-1">
              <Label className="text-xs text-slate-700">Vehicle / Truck No</Label>
              <Input
                value={vehicleNo}
            ...
            <div className="space-y-1">
              <Label className="text-xs text-slate-700">Driver Name / Phone</Label>
              <Input
                value={driverName}
            ...
            <div className="space-y-1">
              <Label className="text-xs text-slate-700">Dispatch Chalan No</Label>
              <Input
                value={chalanNo}
          ...
          {/* Remarks */}
          <div className="space-y-1">
            <Label className="text-xs text-slate-700">Notes & Purpose</Label>
            <Textarea
              value={remarks}
              onChange={(e) => setRemarks(e.target.value)}
              rows={2}
              placeholder="e.g. Surplus cement transferred for retaining wall urgent casting."
              className="text-xs bg-white border-[#c7d8e8] text-slate-900 resize-none"
            />
          </div>

          {/* Footer Actions */}
          <div className="flex items-center justify-between pt-3 border-t border-[#c7d8e8]">
            <div className="text-xs text-muted-foreground flex items-center gap-1.5">
              <Package className="h-3.5 w-3.5 text-[#0284c7]" />
