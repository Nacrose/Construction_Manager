        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-white/10">
          {onCancel && (
            <Button
              ...
              onClick={onCancel}
              className="h-9 px-4 text-xs font-mono bg-transparent border-white/10 hover:bg-white/5 text-gray-300"
            >
              {cancelLabel}
            </Button>
          )}
          ...
          <Button
            ...
            size="sm"
            disabled={isPending || disabled}
            className="h-9 px-5 text-xs font-semibold gap-2 bg-emerald-600 hover:bg-emerald-500 text-white shadow-sm"
          >
  ...
  return (
    <div className={cn("space-y-1.5", colSpanClasses[colSpan], className)}>
      ...
      <Input
        ...
        className={cn(
          "h-9 text-xs bg-[#121820] border-white/10 text-white placeholder:text-gray-500 rounded-xl focus:border-emerald-500",
      ...
      {error ? (
        ...
        <p className="text-[10px] text-gray-400 font-mono">{helperText}</p>
      ) : null}
    </div>
  );
}
...
export function FormNumberField({
  ...
  return (
    <div className={cn("space-y-1.5", colSpanClasses[colSpan], className)}>
      <Label className="text-xs text-gray-300 font-medium flex items-center gap-1">
        {label}
        {unit && <span className="text-gray-400 font-mono text-[10px]">({unit})</span>}
        {required && <span className="text-red-400">*</span>}
      </Label>
      <div className="relative">
        <Input
          ...
          placeholder={placeholder}
          className={cn(
            "h-9 text-xs font-mono bg-[#121820] border-white/10 text-white placeholder:text-gray-500 rounded-xl focus:border-emerald-500",
            unit && "pr-12",
            error && "border-red-500/50 focus:border-red-500"
          )}
        />
        {unit && (
          <span className="absolute right-3 top-2.5 text-[10px] font-mono text-gray-400 pointer-events-none">
            {unit}
          </span>
        )}
      </div>
...
export function FormCurrencyField({
  ...
  return (
    <div className={cn("space-y-1.5", colSpanClasses[colSpan], className)}>
      ...
      <div className="relative">
        <span className="absolute left-3 top-2.5 text-[10px] font-mono text-gray-400 font-semibold pointer-events-none">
          NPR
        </span>
        <Input
          type="number"
          ...
          placeholder={placeholder}
          className={cn(
            "pl-12 h-9 text-xs font-mono bg-[#121820] border-white/10 text-white placeholder:text-gray-500 rounded-xl focus:border-emerald-500",
            error && "border-red-500/50 focus:border-red-500"
          )}
        />
      </div>
  ...
  return (
    <div className={cn("space-y-1.5", colSpanClasses[colSpan], className)}>
      ...
      <NepaliDatePicker
        ...
        disabled={disabled}
        className={cn(
          "w-full h-9 text-xs bg-[#121820] border-white/10 text-white rounded-xl",
  ...
  return (
    <div className={cn("space-y-1.5", colSpanClasses[colSpan], className)}>
      ...
      <Select value={value} onValueChange={(val) => setValue(name, val)} disabled={disabled}>
        <SelectTrigger
          className={cn(
            "h-9 text-xs bg-[#121820] border-white/10 text-white rounded-xl focus:border-emerald-500",
            error && "border-red-500/50"
          )}
        >
  ...
  return (
    <div className={cn("space-y-1.5", colSpanClasses[colSpan], className)}>
      ...
      <Textarea
        ...
        placeholder={placeholder}
        className={cn(
          "text-xs bg-[#121820] border-white/10 text-white placeholder:text-gray-500 rounded-xl focus:border-emerald-500 resize-none",
          error && "border-red-500/50 focus:border-red-500"
        )}
      />
      {error ? (
...
export function FormSwitchField({
  ...
  return (
    <div
      className={cn(
        "flex items-center justify-between p-3 rounded-xl bg-[#121820] border border-white/10",
        colSpanClasses[colSpan],
        className
      )}
    >
      <div className="space-y-0.5">
        <Label className="text-xs text-gray-200 font-medium cursor-pointer">{label}</Label>
        {helperText && <p className="text-[10px] text-gray-400 font-mono">{helperText}</p>}
      </div>
      <Switch
        checked={checked}
