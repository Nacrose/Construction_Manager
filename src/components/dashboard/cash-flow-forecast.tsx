      ...
      <CardContent className="pt-0 space-y-3">
        <div className="grid grid-cols-3 gap-2">
          <div className="rounded border p-2 text-center">
            ...
            <div className="text-sm font-bold text-emerald-600">{npr(s.totalBilled)}</div>
          </div>
          <div className="rounded border p-2 text-center">
            <div className="text-[9px] text-muted-foreground uppercase">Spent</div>
            <div className="text-sm font-bold text-red-600">{npr(s.totalSpent)}</div>
          </div>
          ...
          {data.timeline.map(t => {
            ...
            return (
              <div key={t.month} className="grid grid-cols-4 gap-1 text-[10px] px-1 py-0.5 rounded hover:bg-muted/30">
                ...
                <span className="text-right text-emerald-600 tabular-nums">{t.billed > 0 ? npr(t.billed) : "—"}</span>
                <span className="text-right text-red-600 tabular-nums">{t.spent > 0 ? npr(t.spent) : "—"}</span>
                <span className={cn("text-right tabular-nums font-medium", t.cumulative >= 0 ? "text-emerald-600" : "text-red-600")}>
                  {t.cumulative >= 0 ? "+" : ""}{npr(t.cumulative)}
                </span>
              </div>
            );
          })}
        ...
        {(s.totalRetention > 0 || s.totalAdvanceRecovered > 0) && (
          <div className="flex items-center gap-4 text-[10px] text-muted-foreground border-t pt-2">
            {s.totalRetention > 0 && <span>Retention held: <strong className="text-amber-600">{npr(s.totalRetention)}</strong></span>}
            {s.totalAdvanceRecovered > 0 && <span>Advance recovered: <strong className="text-blue-600">{npr(s.totalAdvanceRecovered)}</strong></span>}
          </div>
        )}
      </CardContent>
    </Card>
